...
import numpy
import math
from cozmo.util import degrees, radians, Pose
...
def get_relative_pose(object_pose, reference_frame_pose):
	...
	angle_deg = reference_frame_pose.rotation.angle_z.degrees # - object_pose.rotation.angle_z.degrees

	new_x = x * math.cos(angle_deg) - y * math.sin(angle_deg) + reference_frame_pose.position.x
	new_y = x * math.sin(angle_deg) + y * math.cos(angle_deg) + reference_frame_pose.position.y

	return Pose(new_x, new_y, 0, angle_z = degrees(angle_deg))
